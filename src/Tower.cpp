...
bool Tower::shouldShoot() { //this effectively creates gaps in time between each bullet shot
    if (towerType == "sanitizer" && shootingGap % 500 == 0) {
        //shootingGap = 0;
        return true;
    } else
        if (towerType == "soap" && shootingGap % 800 == 0) { //soap doesn't shoot as often
            //shootingGap = 0;
            return true;
        } else return false;
}
...
Tower::Tower(sf::Vector2f size, sf::Vector2f coords, std::string type,
             Cell* ground) { //should receive coordinates too
    tower.setSize(size);
